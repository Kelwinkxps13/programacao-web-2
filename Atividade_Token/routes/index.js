...
router.get('/get_token', (req, res, next)=> {
  res.render('token_request', {'title': "pegar token"})
});

router.post('/get_token', async (req, res, next) => {

  var request = new FormData();
  request.append('username', req.body.name); 
  request.append('password', req.body.password); 

  const response = await axios.post('https://mauricio.inf.br/p6/api/auth/', request);
  const token = response.data.access_token
  console.log('token = '+token)

  res.redirect('/?status=tokenPegadoComSucesso');
});
